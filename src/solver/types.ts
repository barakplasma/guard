...
export interface SolverInstance {
  ...
  readonly seatsWanted: readonly (readonly number[])[];
  readonly slotOfSegment: readonly (readonly number[])[];
  readonly holdOfSegment: readonly (readonly number[])[];

  readonly isAvailable: readonly (readonly boolean[])[];
  readonly isAllowed: readonly (readonly boolean[])[];
  ...
  readonly holdsRequirement: readonly (readonly boolean[])[];
...
export interface SegmentDiagnostics {
  ...
  readonly turnsOnMission: readonly (readonly number[])[];
...
export const OBJECTIVE_ORDER = [
  ...
  'missionRepeatCost',
  'nightHourRepeatCost',
] as const satisfies readonly (keyof NamedQuantities)[];
