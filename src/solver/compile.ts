...
export interface LadderParams {
  ...
  readonly candidateAssignment?: ReadonlyArray<readonly number[]>
}
...
export function deriveSymmetryClasses (
  problem: PreparedProblem,
  isAvailable: ReadonlyArray<readonly boolean[]>,
  isAllowed: ReadonlyArray<readonly boolean[]>,
  holdsRequirement: ReadonlyArray<readonly boolean[]>,
  memory: MemoryTables
): number[] {
  ...
  problem.employees.forEach((employee, index) => {
    ...
    if (group != null) group.push(index); else byKey.set(key, [index])
  })
...
export function instanceToJsonData (instance: SolverInstance, ladder: LadderParams): Record<string, unknown> {
  ...
  if (ladder.candidateAssignment != null) data.candidateAssignment = ladder.candidateAssignment
  return data
}
