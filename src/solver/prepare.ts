...
export function resolveNights (draft: Draft): Interval[] {
  const raw: Array<{ start: number, end: number }> = nightWindows(draft)
  const clipped = raw
 ...
 */
function memoryNights (draft: Draft, memoryStart: number): Array<{ start: number, end: number }> {
...
export function readDutyMemory (draft: Draft, memoryDays = DEFAULT_MEMORY_DAYS): Map<EmployeeId, DutyMemory> {
  ...
  for (const turn of turns) {
    ...
    if (at == null) { at = blankTally(); byEmployee.set(turn.employeeId, at) }
...
export function prepareProblem (draft: Draft, clock: { now: number }): PreparedProblem {
  ...
  const grid: Array<{ mission: Draft, segments: Array<{ start: number, end: number, slot: { start: number, end: number } }> }> =
    segmentGrid(input)
