...
export interface TimelineSegment {
  ...
  onDuty: { employeeId: string; missionId: string }[];
  offDuty: string[];
  unavailable: string[];
}
...
export function timelineFrom(
  ...
  return index.segments.map((segment, segmentIndex) => {
    const onDuty: { employeeId: string; missionId: string }[] = [];
...
): { start: number; end: number; got: number }[] {
  const windows: { start: number; end: number; got: number }[] = [];
...
export function statsFrom(
  ...
  return {
    perEmployee,
    spreadMinutes: minutes.length ? Math.max(...minutes) - Math.min(...minutes) : 0,
