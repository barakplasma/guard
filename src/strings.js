...
export const t = {
  ...
  shortShift: (name, mission, actual, expected) => `${name} — ${mission}: משמרת קצרה, ${actual} דקות במקום ${expected}. ייתכן שזה קצה תקופה, זמינות או שיבוץ ידני.`,
  longShift: (name, mission, actual, expected) => `${name} — ${mission}: משמרת ארוכה, ${actual} דקות במקום ${expected}.`,
  workloadOutlier: (name, count, average) => `${name}: ${count} משמרות, יותר מפי שניים מהממוצע (${average}). יש לבדוק זמינות, הסמכות ושיבוצים ידניים.`,
  scheduleChecks: (errors, other) => `בדיקות הסידור · ${errors} בעיות · ${other} לבדיקה`,
  checkAll: 'הכול',
  checkErrors: 'בעיות',
  checkDurations: 'אורך משמרת',
  checkWorkload: 'עומס',
  checkUnused: 'ללא משמרות',
  noMatchingFindings: 'אין התראות בקטגוריה זו.',
  scheduleChecksHelp: 'התראות לבדיקה אינן בהכרח שגיאות. חלון זמן קצר או חלוקת עומס לא שווה יכולים לנבוע מאילוצי הסידור.',
  copyScheduleReport: 'העתקת דוח לבדיקה',

  // the MiniZinc path, shown beside the engine's answer while it is still the
  // authority (ADR 017, migration step 5)
  solverPanel: 'מנוע חלופי (MiniZinc)',
  solverRun: 'הרצת המנוע החלופי',
  solverSolving: 'מחשב…',
  solverOff: 'המנוע החלופי אינו פועל. ההרצה מורידה את רכיב החישוב בפעם הראשונה.',
  solverOptimal: (levels) => `נמצא פתרון מיטבי, ${levels} רמות הוכחו.`,
  solverFeasible: (levels) => `נמצא פתרון, ${levels} רמות הוכחו.`,
  solverInfeasible: 'אין פתרון: אין מספיק אנשים לאיוש המשימות.',
  solverUnknown: (level) => `לא הושלם חישוב ברמה ${level} בזמן שהוקצב.`,
  solverFailed: (reason) => `החישוב נכשל (${reason}).`,
  solverCancelled: 'החישוב בוטל.',
  solverStale: 'התוצאה המוצגת שייכת לגרסה קודמת של הסידור.',
  solverDiff: (same, total) => `התאמה למנוע הקיים: ${same} מתוך ${total} שיבוצים.`,
  typeDaily: 'יומית',
  ...
  hardMission: 'משימה קשה',
  hardMissionHelp: 'משימה שאף אחד לא אוהב, כמו מטבח: היא עוברת על כמה שיותר אנשים לפני שהיא חוזרת למישהו. אורך הסבב נגזר מהרשימה ומהיומן, ואין צורך להגדיר אותו.',
  exemptFromHardMissions: 'פטור ממשימות קשות',
  exemptFromHardMissionsHelp: 'מי שמחזיק בכשירות הזו — נהג או מפקד, בדרך כלל — לא נכלל בסבב של משימות קשות. הוא עדיין יכול לקבל אחת כשאין אף אחד אחר, אבל היעדרותו מהסבב לא מעכבת את השאר.',
  dailyMissingClock: (mission) => `${mission}: יש למלא שעת התחלה ושעת סיום לתורנות.`,
  targetSleepMissed: (person) => `${person}: לא התאפשרו שמונה שעות רצופות של מנוחה בלילה הזה.`,
  tooManyMissions: (count, limit) => `יש ${count} משימות; המנוע תומך בעד ${limit}.`,
  logTrimmed: (n) => `הקישור התמלא: ${n} תורנויות ישנות הוסרו מההיסטוריה. כדאי לייצא את ההיסטוריה כדי לשמור אותה.`,
  ...
  memoryDays: 'זיכרון הסידור (ימים)',
  memoryDaysHelp: 'כמה ימים אחורה נספרת תורנות שכבר בוצעה: למי מגיע התור הבא, חלוקת שעות הלילה, סבב בין המשימות והכלל של "פעם בסבב". התורנויות עצמן נשמרות בקישור ואינן נמחקות.',
