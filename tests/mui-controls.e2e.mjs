/** Acceptance coverage for searchable 18-person rosters and standard MUI disclosure/pickers. */
...
try {
  ...
  assert.equal(await page.getByRole('option').count(), 1, 'search filters 18 people');
  ...
  assert.equal(await replacement.getAttribute('role'), 'combobox');
  await replacement.fill('זוהר');
  assert.equal(await page.getByRole('option').getAttribute('aria-disabled'), 'true');
  await replacement.fill('אלון');
  ...
  console.log('PASS 18-person search, partial pins, unavailable replacements, swaps, accordion keyboard/ARIA, responsive layouts');
} catch (error) {
