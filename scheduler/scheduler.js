 ...
 * @param {Record<string, {start:number,end:number}[]>} [params.unavailablePeriods]
 *   Vacation periods keyed by stable guard id. A guard is unavailable only
 *   for blocks that overlap one of their periods.
...
export function generateShifts({
  ...
  for (const [guard, periods] of Object.entries(unavailablePeriods)) {
    ...
    for (const period of periods) {
      if (!(period?.end > period?.start)) {
        throw new Error(`Unavailable period for ${guard} must have an end after its start.`);
      }
    }
  }
  ...
  for (const demand of demands) {
    ...
    const candidates = [...state.values()]
      .filter((s) => s.busyUntil <= ref)
      .filter((s) => !(unavailablePeriods[s.name] || [])
        .some((period) => period.start < demand.blockEnd && period.end > demand.blockStart));
